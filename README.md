<h1>안녕하세요, 이은지입니다.</h1>
<h3>안전한 연결, 신뢰의 세상을 만들고 싶은 사이버 보안학과 학생입니다.</h3>

<table border="0">
<tr>
<td>

안심할 수 있는 디지털 환경 구축을 목표로 학습 중인 학생입니다. <br>다양한 보안 프로젝트와 실습을 통해 취약점 분석 및 침해 대응 능력을 쌓았습니다.
<br>여러 대외 활동으로 경청과 명확한 의견 제시 능력을 키워 다양한 배경의 팀원들과 성공적으로 소통하며 원활한 협업 환경을 조성하는 능력이 장점입니다.
<br>전문성과 실전 역량을 동시에 갖춘 보안 인재로 성장하는 것을 목표로 하고 있습니다.

</td>
<td>
<img src="https://github.com/user-attachments/assets/92a32b24-bcb0-4c3c-97fd-df9af31eabe2" alt="profile"/>
</td>
</tr>
</table>


<img src="https://github.com/user-attachments/assets/92a32b24-bcb0-4c3c-97fd-df9af31eabe2" width="180px" align="right" style="padding-left: 20px; border-radius: 10px;"/>
다양한 보안 프로젝트와 실습을 통해 취약점 분석 및 침해 대응 능력을 쌓았습니다.<br>
저는 능동적인 경청과 명확한 의견 제시를 통해 팀의 시너지를 이끌어냅니다. 
다양한 배경의 팀원들과 성공적으로 소통하며 원활한 협업 환경을 조성하는 능력이 장점입니다.<br>
전문성과 실전 역량을 동시에 갖춘 보안 인재로 성장하는 것을 목표로 하고 있습니다.


---

## 목차
- Profile
- [Learning Goal](https://github.com/eunjileeDev/my_resume?tab=readme-ov-file#-1)
- [Tech Stack](https://github.com/eunjileeDev/my_resume?tab=readme-ov-file#-2)
    - [Languages](https://github.com/eunjileeDev/my_resume?tab=readme-ov-file#--languages)
    - [Tools/Development-Environment](https://github.com/eunjileeDev/my_resume?tab=readme-ov-file#--toolsdevelopment-environment)
    - [Frameworks](https://github.com/eunjileeDev/my_resume?tab=readme-ov-file#--frameworks)
    - [Security Tools](https://github.com/eunjileeDev/my_resume?tab=readme-ov-file#--security-tools-packet-capture-collection-analysis-and-deletion-etc)
- [Projects](https://github.com/eunjileeDev/my_resume?tab=readme-ov-file#-3)
    - [Team Projects](https://github.com/eunjileeDev/my_resume?tab=readme-ov-file#-3)
    - [Personal Projects](https://github.com/eunjileeDev/my_resume?tab=readme-ov-file#-3)
- [대외활동](https://github.com/eunjileeDev/my_resume?tab=readme-ov-file#-4)

## [![Typing SVG](https://readme-typing-svg.demolab.com?font=Stack+Sans+Text&size=27&pause=1000&color=008000&width=435&lines=%F0%9F%A7%91%E2%80%8D%F0%9F%92%BBProfile)](https://git.io/typing-svg)

|  |  | 
| :--- | :--- | 
| **이름** | 이은지(Lee eunji) | 
| **전공** | 단국대학교 산업보안학과 | 
| **연락처** | enji4882@gmail.com | 
| **GitHub** | [GitHub Repository](https://github.com/eunjileeDev) |
| **관심 분야** | 네트워크 보안, 취약점 분석 | 

---
## <br>[![Typing SVG](https://readme-typing-svg.demolab.com?font=Stack+Sans+Text&size=27&pause=1000&color=008000&width=435&lines=%F0%9F%8E%AFLearning+Goal)](https://git.io/typing-svg)

저는 현재 언어 능력을 향상시키는 동시에 다양한 도구와 기술을 직접 사용해보며 폭넓은 역량을 쌓는 것을 목표로 하고 있습니다.  
여러 사람과 함께 프로젝트를 수행하며 실제 협업 경험을 쌓고, 네트워크 보안, 취약점 분석, 클라우드 보안 등 다양한 분야를 탐색하면서 자신에게 가장 적합한 직무를 찾아가는 과정에 집중하고 있습니다.  

이를 통해 단순한 기술 습득을 넘어, 실무에서 활용 가능한 전문성과 문제 해결 능력을 갖춘 개발자로 성장하고자 꾸준히 학습과 경험을 이어가고 있습니다.



## <br>[![Typing SVG](https://readme-typing-svg.demolab.com?font=Stack+Sans+Text&size=27&pause=1000&color=008000&width=435&lines=%F0%9F%92%BBTech+Stack)](https://git.io/typing-svg)

### - Languages
- [![C](https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white)](https://www.cprogramming.com/)
[![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)](https://www.python.org/)
- [![C++](https://img.shields.io/badge/C++-A0A0A0?style=for-the-badge&logo=c%2B%2B&logoColor=white)](https://en.cppreference.com/w/cpp) 
 [![JavaScript](https://img.shields.io/badge/JavaScript-A0A0A0?style=for-the-badge&logo=javascript&logoColor=black)](https://developer.mozilla.org/ko/docs/Web/JavaScript)
 [![HTML5](https://img.shields.io/badge/HTML5-E0E0E0?style=for-the-badge&logo=html5&logoColor=white)](https://developer.mozilla.org/ko/docs/Web/HTML) 
 [![CSS3](https://img.shields.io/badge/CSS3-D0D0D0?style=for-the-badge&logo=css3&logoColor=white)](https://developer.mozilla.org/ko/docs/Web/CSS) 

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=eunjileeDev&layout=compact&theme=radical)


### - Tools/Development Environment
- [![Linux](https://img.shields.io/badge/Linux-000000?style=for-the-badge&logo=linux&logoColor=white)](https://www.kernel.org/)  [![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)](https://git-scm.com/)
 [![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)](https://www.mysql.com/)
 [![GDB](https://img.shields.io/badge/GDB-4E8CCB?style=for-the-badge&logo=gdb&logoColor=white)](https://sourceware.org/gdb/)

### - FrameWorks
- [![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js&logoColor=white)](https://nodejs.org/)
- library(data visualization)
    - [![Pandas](https://img.shields.io/badge/Pandas-A0A0A0?style=for-the-badge&logo=pandas&logoColor=white)](https://pandas.pydata.org/)  [![Matplotlib](https://img.shields.io/badge/Matplotlib-A0A0A0?style=for-the-badge&logo=matplotlib&logoColor=white)](https://matplotlib.org/) [![Seaborn](https://img.shields.io/badge/Seaborn-A0A0A0?style=for-the-badge&logo=seaborn&logoColor=white)](https://seaborn.pydata.org/) 

### - Security Tools (packet capture, collection, analysis, and deletion etc.)
- [![Wireshark](https://img.shields.io/badge/Wireshark-1D3557?style=for-the-badge&logo=wireshark&logoColor=white)](https://www.wireshark.org/)   [![Burp Suite](https://img.shields.io/badge/Burp_Suite-FF6A00?style=for-the-badge&logo=portswigger&logoColor=white)](https://portswigger.net/burp)
 [![FTK_Imager](https://img.shields.io/badge/FTK_Imager-0052A5?style=for-the-badge)](https://www.exterro.com/digital-forensics-software/ftk-imager) 
 [![CyLR](https://img.shields.io/badge/CyLR-FF8C00?style=for-the-badge&logo=github&logoColor=white)](https://github.com/orlikoski/CyLR) 
 [![Volatility](https://img.shields.io/badge/Volatility-4C4CFF?style=for-the-badge&logo=python&logoColor=white)](https://www.volatilityfoundation.org/) 
 [![Redline](https://img.shields.io/badge/Redline-FF3333?style=for-the-badge&logo=fireeye&logoColor=white)](https://www.fireeye.com/services/freeware/redline.html) 
 [![Eraser](https://img.shields.io/badge/Eraser-A0A0A0?style=for-the-badge)](https://eraser.heidi.ie/) 
 [![RegEdit](https://img.shields.io/badge/RegEdit-A0A0A0?style=for-the-badge)](https://learn.microsoft.com/en-us/troubleshoot/windows-server/performance/windows-registry-advanced-users)
---

## <br>[![Typing SVG](https://readme-typing-svg.demolab.com?font=Stack+Sans+Text&size=27&pause=1000&color=008000&width=435&lines=%F0%9F%9B%A1%EF%B8%8FProjects)](https://git.io/typing-svg)

### - Team Projects

<table border="1" cellspacing="0" cellpadding="5">
  <tr>
    <td>
      <a href="https://github.com/system-security-0x/ransonware-battle/tree/main/test/test_result">
        <img src="https://img.shields.io/badge/View%20on-GitHub-181717?style=for-the-badge&logo=github" alt="View on GitHub">
      </a>
    </td>
    <td>
      <b><i>fuse filesystem이용한 안티 랜섬웨어 개발</i></b>
    </td>
  </tr>
  <tr>
    <td><b>Description</b></td>
    <td>리눅스의 fuse filesystem을 이용해 PID 기반 행동 추적, 악성 행동 감지, 엔트로피 분석, 화이트리스트/블랙리스트, 자동 차단 등으로 랜섬웨어 방어</td>
  </tr>
  <tr>
    <td><b>Technologies</b></td>
    <td>C, linux</td>
  </tr>
  <tr>
    <td><b>Contributions</b></td>
    <td><a href="https://github.com/eunjileeDev/test1.git">기본 fuse filesystem, pid기반 행동 추적, 자동 차단 구현 <br>(개인repository)</td>
  </tr>
</table>

<table border="1" cellspacing="0" cellpadding="5">
  <tr>
    <td>
      <a href="웹프로그래밍/텀프로젝트">
        <img src="https://img.shields.io/badge/View%20on-GitHub-181717?style=for-the-badge&logo=github" alt="View on GitHub">
      </a>
    </td>
    <td>
      <b><i>API 사용해 위치, 공연 정보, 검색 기능 제공 웹 개발</i></b>
    </td>
  </tr>
  <tr>
    <td><b>Description</b></td>
    <td>공공API를 사용해 영화관 위치, 영화 상세 정보, 종합 공연정보, 키워드 도서 검색 기능 제공</td>
  </tr>
  <tr>
    <td><b>Technologies</b></td>
    <td>CSS, HTML, JavaScript</td>
  </tr>
  <tr>
    <td><b>Contributions</b></td>
    <td>예술의 전당 종합 공연 정보 제공, 키워드로 도서 검색, 통합</td>
  </tr>
</table>

<table border="1" cellspacing="0" cellpadding="5">
  <tr>
    <td>
      <a href="대학기초SW입문/팀 프로젝트">
        <img src="https://img.shields.io/badge/View%20on-GitHub-181717?style=for-the-badge&logo=github" alt="View on GitHub">
      </a>
    </td>
    <td>
      <b><i>크롤링 이용한 코디 프로그램 개발</i></b>
    </td>
  </tr>
  <tr>
    <td><b>Description</b></td>
    <td>크롤링해 날씨를 가져와 날씨에 맞게 사용자 옷장의 옷 코디 제공</td>
  </tr>
  <tr>
    <td><b>Technologies</b></td>
    <td>Python, Tkinter, urllib,  BeautifulSoup</td>
  </tr>
  <tr>
    <td><b>Contributions</b></td>
    <td>네이버에서 서울의 기온을 크롤링하여 가져오기 구현</td>
  </tr>
</table>

### - Personal Projects

<table border="1" cellspacing="0" cellpadding="5">
  <tr>
    <td>
      <a href="운영체제/랜섬웨어 프로젝트/dkuware">
        <img src="https://img.shields.io/badge/View%20on-GitHub-181717?style=for-the-badge&logo=github" alt="View on GitHub">
      </a>
    </td>
    <td>
      <b><i>AES-128 기반 랜섬웨어 공격/복구 시뮬레이터</i></b>
    </td>
  </tr>
  <tr>
    <td><b>Description</b></td>
    <td>멀티스레딩을 활용해 target/ 디렉터리 내의 JPG, PDF 파일을 탐색 및 처리, 파일의 헤더 16바이트를 XOR 연산으로 손상시키고 복호화에 필요한 mask 값을 AES-128로 암호화해 파일 푸터에 저장하는 키 관리 메커니즘 구현</td>
  </tr>
  <tr>
    <td><b>Technologies</b></td>
    <td>C</td>
  </tr>
</table>

<table border="1" cellspacing="0" cellpadding="5">
  <tr>
    <td>
      <a href="리눅스기초/라즈베리파이 개인 프로젝트">
        <img src="https://img.shields.io/badge/View%20on-GitHub-181717?style=for-the-badge&logo=github" alt="View on GitHub">
      </a>
    </td>
    <td>
      <b><i>라즈베리파이 텔레그램 봇</i></b>
    </td>
  </tr>
  <tr>
    <td><b>Description</b></td>
    <td>텔레그램으로 명령어를 보내면 라즈베리파이 초음파센서로 거리를 측정해 전송하고 일정 거리 이하이면  감시용 스레드를 실행해 지속 감시, 촬영한 사진 전송</td>
  </tr>
  <tr>
    <td><b>Technologies</b></td>
    <td>Python, linux, telegram bot, Raspberry Pi</td>
  </tr>
</table>

<table border="1" cellspacing="0" cellpadding="5">
  <tr>
    <td>
      <a href="데이터사이언스/개인 프로젝트">
        <img src="https://img.shields.io/badge/View%20on-GitHub-181717?style=for-the-badge&logo=github" alt="View on GitHub">
      </a>
    </td>
    <td>
      <b><i>업종별 산재현황과 부동산 가격 관계 DB분석, 시각화</i></b>
    </td>
  </tr>
  <tr>
    <td><b>Description</b></td>
    <td>업종별 산재현황과 부동산 실거래가와의 관계를 공공 DB를 이용해 여러 분석도구로 분석, 시각화</td>
  </tr>
  <tr>
    <td><b>Technologies</b></td>
    <td>Python, seaborn lib, hitmap, bubble chart, Scatter Plot, Correlation Coefficient</td>
  </tr>
</table>

<table border="1" cellspacing="0" cellpadding="5">
  <tr>
    <td>
      <a href="융합코딩2/개인프로젝트">
        <img src="https://img.shields.io/badge/View%20on-GitHub-181717?style=for-the-badge&logo=github" alt="View on GitHub">
      </a>
    </td>
    <td>
      <b><i>Text기반 RPG 게임</i></b>
    </td>
  </tr>
  <tr>
    <td><b>Description</b></td>
    <td>전투, 레벨업, 수집, 맵 탐험등 포인터를 사용한 동적할당을 중심으로 게임 구현</td>
  </tr>
  <tr>
    <td><b>Technologies</b></td>
    <td>C</td>
  </tr>
</table>

<table border="1" cellspacing="0" cellpadding="5">
  <tr>
    <td>
      <a href="창의적사고와코딩/개인 프로젝트">
        <img src="https://img.shields.io/badge/View%20on-GitHub-181717?style=for-the-badge&logo=github" alt="View on GitHub">
      </a>
    </td>
    <td>
      <b><i>복권 번호 생성, 등수 확인 구현</i></b>
    </td>
  </tr>
  <tr>
    <td><b>Description</b></td>
    <td>복권 번호를 생성하고, 로또 사이트에서 회차를 입력하면 번호를 크롤링해 와 사용자의 등수를 제공</td>
  </tr>
  <tr>
    <td><b>C, urllib.request, BeautifulSoup</b></td>
    <td>C</td>
  </tr>
</table>

--- 
## <br>[![Typing SVG](https://readme-typing-svg.demolab.com?font=Stack+Sans+Text&size=27&pause=1000&color=008000&width=435&lines=%F0%9F%8F%86Extracurricular+Activities)](https://git.io/typing-svg)

- **개발/보안 동아리 (Aegis)** (2023.03~2025.06)  
    - C언어와 Java 프로그래밍 팀 스터디 참여를 통해 학습 및 토론 진행
    - 전문가 초청 강연을 통해 실무 지식과 보안 인식 향상 
- **요트부 동아리(dkyc) 집행부 기획부장** (2024.03~2025.04)
    - 딩기요트 조작 및 항해 참여
    - 여러 대회와 여름/겨울 해양훈련 참여를 통해 협동심과 문제 해결력 향상
    - 기획부장으로서 학기별 활동 및 훈련 일정 기획
- **봉사 동아리** (2024.09~2025.06)
    - 교내외 다양한 봉사 활동에 참여

- **교내 근로 장학생** (2023/24.09 ~ 01, 2025.09~)
    - 사무 행정 보조 및 문의·전화 응대 업무
    - 연구 관련 자료 및 파일 정리 업무 담당  
    - 교내 박물관 고고·복식 분야 실무 보조

--- 


